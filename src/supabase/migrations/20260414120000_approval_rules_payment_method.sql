-- ============================================================
-- Metode pembayaran pada approval_rules (whitelist + blacklist)
-- ============================================================

ALTER TABLE approval_rules
  ADD COLUMN IF NOT EXISTS payment_methods text[],
  ADD COLUMN IF NOT EXISTS excluded_payment_methods text[];

COMMENT ON COLUMN approval_rules.payment_methods IS
  'Jika tidak kosong: expense harus punya payment_method yang sama dengan salah satu elemen (exact match). Kosong/NULL = tidak filter whitelist.';
COMMENT ON COLUMN approval_rules.excluded_payment_methods IS
  'Jika tidak kosong: expense dengan payment_method di daftar ini tidak cocok. Untuk "semua kecuali Petty Cash" isi hanya kolom ini.';
